"""Dashboard analytics, extracted from the route handlers so the same
calculations feed both the JSON endpoints and the PDF/CSV report.

These are plain functions (no FastAPI dependencies): pass an open Session and
the resolved account owner.
"""
...
def compute_stats(
    ...
    inactive_total = len(inactive_clients)
...
def compute_hourly_distribution(
    ...
    """Appointments grouped by hour of day over the period, so the report can
    point out the busiest and quietest times. Cancelled ones are ignored."""
    ...
    # only return hours that actually have appointments, ordered by hour
    return [
        {"hour": hour, "label": f"{hour:02d}h", "agendamentos": counts[hour]}
